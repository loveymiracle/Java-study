...
// for loop 네이밍 다는 방법 / flag(깃발) 기법 -> for algorithm
public class BranchStatement2 {
	
	public static void main(String[] args) {
		// 반복문 예시 : 중첩된 반복문을 구현하다 보면, 안쪽에 반복문에서 외부 반복문을 종료시켜야 할 때가 있다.
		
		// 가장 비효율 + 좋지 않은 방법 = 내부종료조건을 외부에서도 체크한다.
		for(int i =0; i < 5; i++) {
			for(int j = 0; j < 5; j++) {
				if(i == 3) {
					break;
	...
	loop1 : for(int i = 0; i < 5 ; i++) { // 외부 반복문의 별칭 : loop1, 내부도 사용할 수 있다.
		for(int j = 0; j < 5; j++) {
			if(i == 3) {
				break loop1;
			}
	...
	// -> 고전적인 기법, 알고리즘적으로 깔끔하게 구현할 수 있어서 선호하는 사람이 많다.
