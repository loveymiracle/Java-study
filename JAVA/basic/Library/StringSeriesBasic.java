...
public class StringSeriesBasic {
	...
	// 4. String의 덧셈 연산의 예시 -> 연산도 많아지고, 메모리 낭비도 심하다.
	// String str = "a" + "b" + "c" + "d"; // 메모리 크기 : 4 , 문자열 하나를 1개로 취급 (4*4)
	// Step 1) "a" + "b" -> "ab"		   // 메모리 크기 : 4 + 2 = 6
	// Step 2) "ab" + "c" -> "abc"         // 메모리 크기 : 6 + 3 = 9 (9*3)
	// Step 3) "abc" + "d" -> "abcd"       // 메모리 크기 : 9 + 4 = 13,
	
	// StringBuffer의 덧셈 연산의 예시
	// append("a"), append("b"), append("c"), append("d") //4
	// 1) [a ... ] -> ['a', 'b' ....]                     // 메모리 크기 : 2
	// 2) ['a', 'b' ....] -> ['a','b', 'c', ....]		  // 메모리 크기 : 3
	// 3) ['a', 'b', 'c' ....] -> ['a', 'b', 'c', 'd']	  // 메모리 크기 : 4
	public static void main(String[] args) {
		// 0. 선언 및 출력하기
		StringBuffer sb = new StringBuffer("안녕하세요?"); // Buffer = 양동이 /공간, 버퍼링, 예비공간
//		StringBuffer sb = new StringBuilder("안녕하세요?"); // StringBuffer와 문법이 완벽히 호환 된다.
		System.out.println(sb); // toString이 생략
		System.out.println(sb.toString());
		
		// 1. append : 문자열 추가하는 기능 ✭✭✭✭✭
//		String str = str.concat("~~"); // 문자열 메커니즘 -> 변수로 받아야 했었다
		// StringBuffer 메소드 메커니즘은 자기 자신에 덧셈과 연산이 더해진다.
		sb.append(" 저는 홍길동입니다.");
		...
		System.out.println(index); // 0, 양수 = 문자열에 있을 때
		index = sb.indexOf("김길동");
		System.out.println(index); // 음수 = 문자열이 없을 때
		...
		sb.delete(index2, index2 + 3); // 시작점, 끝점 (하드코딩)
//		sb.delete(index2, index2 + "홍길동".length());
		System.out.println(sb);
		
		// 4-1. 삭제한 곳에 다른 이름 추가하기, 참고로 replace 제공하지 않아서 이런 식으로 진행 해야 함
