...
public class String2 {
	public static void main(String[] args) {
		...
		//    equalsIgnoreCase : 대소문자 비교를 무시하는 비교
		...
		// 2. contains : 문자열의 포함여부를 알려주는 메소드
		// -> 한글 허용되는 방법
		System.out.println(a.contains("a")); // 포함! -> true
		System.out.println(a.contains("c")); // 미포함! -> false
		System.out.println(a.contains(b)); // 미포함! -> false
		System.out.println("aaabbb".contains("aaa")); // 포함! -> true
		System.out.println(b.contains(a)); // 포함! -> true
		
		// 3. compareTo : 문자열의 대소비교를 해준다. -> 사전순으로 비교해준다.
//		ex) 김길동 > 박길동 > 홍길동 > 홍길홍, 알파벳 순으로도 비교 가능하다.
		// ✭✭✭✭✭, 정렬할 때 필수적으로 외워야 하는 문법
		...
		// 같으면 0, 크면 양수, 작으면 음수
		// 여기서의 비교 숫자는 문자열간의 거리 차이
		
		
		// 4. concat : 문자열을 결합하는 함수
		String str1 = "홍길동";
		String str2 = "의 주소지는 서울입니다.";
		str1.concat(str2); // ??? -> 결합이 되지 않는 문법!!!
		String str3 = str1.concat(str2); // 문자열에서 메소드를 활용하는 정석적인 방법
		System.out.println(str3); 
		String str4 = str1 + str2; // concat과 같은 기능!!
		System.out.println(str4);
		System.out.println("-----------------------------------------------");
		// ※String Lib. 사용 시 주의점
		// - 일반 객체 설계 메커니즘과 다른점이 존재, 기능을 적용해서 본인은 바뀌지 않고 바뀐값을 리턴
		// - this(본인)은 불변하는 원칙 (immutable = 불변성) 떄문이다.
		// - 바뀐 값을 다시 문자열로 받아야 메소드 기능을 활용할 수 있음.
		
		// 5. charAt() : 문자열에서 index에 해당하는 문자를 리턴하는 함수
		String temp = "abcdef";
		...
		// 6. indexOf() : 문자열 내에서  인자의 문자열과 일치하는 index를 반환하는 함수
		//                fromIndex를 활용하면 해당 인덱스 다음으로부터 일치하는 문자열을 찾을 수 있다.
		String str5 = "안녕하세요? 저는 홍길동입니다.";
		//             0 1 2345678 910
		System.out.println(str5.indexOf("안녕")); // index : 0, 첫글자
		System.out.println(str5.indexOf("홍길동")); // index : 10, 10번째 부터 홍길동이 시작됨
		System.out.println(str5.indexOf("하이")); // 없는 문자열!! : -1, 음수면 값을 찾을 수 없을 때
		System.out.println("-----------------------------------------------");
...
//				012345678
		...
		// 그 다음 abc를 찾고 싶을 때? -> fromIndex자리 활용
		System.out.println(str5.indexOf("abc", 1)); // 기본적으로 0만 아니면 다음 abc를 찾을 수 있다.
		System.out.println(str5.indexOf("abc", 2)); 
		// 다음 index를 찾아오기 위한 관례적인 문법(효율적인 문법)
		int index = str5.indexOf("abc"); // 기존에 찾은 인덱스 값, = 0;
		System.out.println(str5.indexOf("abc", index + "abc".length()));
		System.out.println();
		System.out.println("----------------------------------------------");
		
		// 응용편
		// '안녕'이라고 친 문자열 개수를 찾고 인덱스도 모두 출력하세요! = indexOf 활용해서
		String str6 = "안녕하세요? 저는 홍길동입니다. 다들 안녕하셨는지요? "
					+ "안녕하지 못하셨으면 죄송하고 안녕히 계세요. 저는 안녕을 찾으러 갑니다.";
		System.out.println(str6.indexOf("안녕")); // 0
		System.out.println(str6.indexOf("안녕", 1)); // 21 ?
		System.out.println(str6.indexOf("안녕", 2)); // 21 ?
		int index1 = str6.indexOf("안녕");
		System.out.println(str6.indexOf("안녕", index + "안녕".length())); // 21 ?
		System.out.println();
		System.out.println("----------------------------------------------");
		
		String token = "안녕"; // 찾을 문자열 = 문자열 작은 덩어리
		int startIndex = 0;
		int count = 0; // 찾은 갯수
		// 알고리즘 상 반복문 3개가 중첩된 형태로 나와야 한다. -> 3차원으로 연산된다.
		...
		// 9. endsWith : 문자열의 끝 패턴을 확인, 주로 확장자를 확인할 때 쓰는 용도 ✭✭✭✭✭
		str7 = "test.txt";
		System.out.println(str7);
		...
		// 10. trim 류 : white space를 제거하는 기능, - strip(since: java version 11)을 권장 ✭✭✭✭✭
		// white space란 : 문자열의 노이즈로, 사람 눈에 보이지 않는 문자열, 공백, newline, tab등 정보로 필요없는 데이터
		...
		// 이름 : #*홍길동, 나이 : #*23, 주소 : #*서울시 강남
		
		// 11. subString : 문자열 지정한 위치로 자를 때 활용
		System.out.println("---------------------------------------------");
		System.out.println(text.substring(10)); // 앞에서 부터 10글자 자르는 방법, 10글자 이후만 남는다.
		System.out.println(text.substring(4, 10)); // 4 ~ 10글자만 잘라서 쓰는 방법
		System.out.println(text.substring(text.length() - 10, text.length())); // 뒤에 10글자만 제거해서 가져오는 방법
		System.out.println("---------------------------------------------");
		
		// 12. replace : 특정 문자열을 교체할 때 활용, 교체를 빈문자열로 할 경우는 제거
		//               -> 모든 문자열이 교체되거나 제거됨 (첫번째만 되지 않음!!!)
//		"".replace(oldChar=변경 할 문자열 , newChar = 변경 되는 문자열)
		System.out.println(text.replace("홍길동", "박길동")); // 기존 홍길동 -> 박길동으로 교체
		System.out.println(text.replace("#*", "")); // #* 문자열을 제거
		
		// 13. replaceAll : 일반 문자열도 가능하지만, 정규식 패턴으로 문자열을 교체/제거 할 때 활용 된다.
		// -> id, pw 또는 입력 값의 특수 문자를 제거하거나 교체하는 용도로도 활용
		...
		// 14. replaceFirst : 처음 문자만 교체할 때
		String str = "AAABBBCCC";
		System.out.println(str.replace("A", "Z"));
		System.out.println(str.replaceFirst("A", "Z"));
		System.out.println("--------------------------------------------");
		
		text = text.replace("#*", ""); // 필요없는 특수문자 제거
		System.out.println(text);
		// 이름 : 홍길동, 나이 : 23, 주소 : 서울시 강남구
		
		
		// 15. split : 문자열을 특정 인자(Delimiter=구분자, 정규식)로 잘라서 토큰(Token)배열로 반환하는 함수
...
//			System.out.println(array2[1].strip()); // 정보만 가져오는 방법
		...
		// 16. join : 배열 된 문자열을 하나의 문자열로 합칠 때 사용
		...
		// 17.StringTokenizer : Split 객체 버전
		System.out.println(text);
		StringTokenizer tokens = new StringTokenizer(text, ",");
		// while문 has + next 패턴
		while(tokens.hasMoreElements()) { // 엘리먼트가 더 있을 때
		...
		// 21. format : %d, %f, %s 등 format 출력 지원하는 함수
		//    -> 소수점 자르는 용도로 많이 활용
