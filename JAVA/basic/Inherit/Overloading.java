...
public class Overloading {
	...
	void myMethod(int val) {
		System.out.println("myMethod 호출! " + val);
	}
	
	// 리턴 type만 달라지면 -> NG!!!
	// Duplicate method myMethod(int) in type Overloading
//	int mymethod(int val) {
//		System.out.println("myMehtod 호출! "  + val);
//	}
	
	// 인자가 많아지는 경우 -> OK!
	void myMethod(int val, int val2) {
		System.out.println("myMethod 호출! " + val + "," + val2);
	}
	...
	public static void main(String[] args) {
		...
		// 만일 오버로딩이 안된다면? -> type 별로 함수의 이름을 바꿔서 활용해야한다.
...
//		System.out.printlnFOrStr("안녕");
	...
	// 최대 인자를 가지는 메소드 = 3개, ✭주기능 메소드
	public void print(String name, String age, String address) {
		if(name == null || name.length() == 0) { // 빈 문자열 체크하는 방법
			name = "홍길동";
		}
		if(age == null || age.length() == 0) { // 빈 문자열 체크하는 방법
			name = "21";
		}
		if(address == null || address.length() == 0) { // 빈 문자열 체크하는 방법
			name = "서울시 강남구 역삼동";
		}
		System.out.println(name + ", " + age + ", " + address);
	}
	
	// 유지 보수 생각 없는 코딩 -> 코드를 중복으로 만들어서 관리한다.
...
//	}
	...
	public void print(String name, String age) {
		print(name, age, null); // 인자가 가장 많은 메소드로 호출하여 예외처리한다.
	}
	
	public void print(String name) {
		print(name, null, null); // 인자가 가장 많은 메소드로 호출하여 예외처리한다.
	}
}
