...
public class MemberController {
	...
	public String memberServlet(HttpServletRequest req, HttpServletResponse resp, HttpSession session) {
		...
		// 세션 가져오는 방법 2 Spring에서 session을 주입 받는 방법 ★★★★★
	...
	// - @RequestParam(value="form의 name") : 파라메터를 받아오는 방법
...
//			@RequestParam(value="devLang") List<String> devLang,  // 컬렉션으로 받을수 있다.
			@RequestParam(value="devLang", required = false) String[] devLang  // 배열로 받는게 기본적이다.
			) {
		Member member = new Member();
		...
		// addAttribute : key-value값으로 request에서 사용했던 패턴 그대로 활용
	...
	public String memberParams(Model model, String id, String name, String gender, int age,
//								@RequestParam List<String> devLang // list로 받을 경우 @RequestParam 생략 불가
								String[] devLang ) {
	...
	// - 자바의 객체와 From의 name을 일치시켜 파라메터를 한번에 가져오는 방법
	// - 반드시 값간의 이름과 type 일치해야 자동으로 맵핑된다. (List - X, String[] 가능)
	...
	public String memberCommand2(Model model, Member member) {
	...
	public String memberCommand(@ModelAttribute("member") Member member) {
		return "member/memberView";
	}
	...
	String memberMap(Model model, @RequestParam Map<String, Object> param, String[] devLang) { //@Requestparam List<String> devLang
	...
	public ModelAndView memberEnroll(ModelAndView model, Member member) {
		...
		if(result > 0) { // 성공
			...
			model.setViewName("redirect:error.do"); // 에러페이지로 리다이렉트하는 코드
		}
		return model;
	}
	
	// 7. List를 통해 회원정보 리스트 보기
